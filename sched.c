...
/* 2017 */
#ifndef TICK_MS_CON_MACROS
#define TICK_MS_CON_MACROS
...
struct runqueue {
	...
	task_t *curr, *idle;
	/* 2017 HW2 changes: */
	prio_array_t *active, *expired, *short_p, *overdue_p, arrays[4];
	/* 2017 end HW2 changes: */
...
static inline void activate_task(task_t *p, runqueue_t *rq)
{
	...
	/* 2017 HW2 changes: */
        ...
        prio_array_t *short_arr = rq->short_p;
        prio_array_t *overdue_arr = rq->overdue_p;

	/* The given process- p, is short and not overdue: */
        if ((p->policy == SCHED_SHORT) && (p->is_overdue == 0)) {
	    enqueue_task(p, short_arr);
	    rq->nr_running++;
	    return;
	}

	/* The given process- p, is an overdue short process: */
        if ((p->policy == SCHED_SHORT) && (p->is_overdue == 1)) {
	    enqueue_task(p, overdue_arr);
	    rq->nr_running++;
	    return;
	}

	/* 2017 HW2 end changes. */
...
void wake_up_forked_process(task_t * p)
{
	...
	p->state = TASK_RUNNING;
	/* 2017 HW2 changes: */
	/* Added "&& (p->policy != SCHED_SHORT)" to the if statement below: */
	if (!rt_task(p) && (p->policy != SCHED_SHORT)) {
	/* 2017 HW2 end changes. */
		/*
	...
	/* The given process is SHORT: */
	} else {
	    prio_array_t *short_arr = rq->short_p;
            prio_array_t *overdue_arr = rq->overdue_p;

	    /* The given process a regular SHORT: */
	    if (p->is_overdue == 0) {
	        /* Current process (the father of the given process p) is a regular SHORT: */
	        if (current->is_overdue == 0) {
                    list_del(&current->run_list);
		    list_add_tail(&current->run_list, short_arr->queue + current->static_prio);//??
		}
	        /* Do not use enqueue here. Priority is static: */
	        list_add(&p->run_list, short_arr->queue + p->static_prio);
		__set_bit(p->static_prio, short_arr->bitmap);
		p->array = short_arr;
		short_arr->nr_active++;
		rq->nr_running++;
		set_tsk_need_resched(current);
	    /* The given process an overude SHORT: */
	    } else {
	        list_add(&p->run_list, overdue_arr->queue + 139);
		__set_bit(139, overdue_arr->bitmap);
		p->array = overdue_arr;
		overdue_arr->nr_active++;
		rq->nr_running++;
		set_tsk_need_resched(current);
	    }
	}
...
void scheduler_tick(int user_tick, int system)
{
	 ...
	 */
        ...
        prio_array_t *active_arr = rq->active;
        //prio_array_t *expired_arr = rq->expired;
	prio_array_t *short_arr = rq->short_p;

	/* The given process is a regular short process (not overdue): */
        if ((p->policy == SCHED_SHORT) && (p->is_overdue == 0)) {

	    /* The short process ran the entire requested time: */
	    if (!--p->time_remained_as_short) {
	        dequeue_task(p, rq->short_p);
		set_tsk_need_resched(p);
		p->prio = 139;
                p->is_overdue = 1;
		p->time_running_as_overdue = 0;
	        enqueue_task(p, rq->overdue_p);
		goto out;
	    }
	}


        /* The given process is an ovedue short process: */
        if ((p->policy == SCHED_SHORT) && (p->is_overdue == 1)) {

	    /* The overdue process has been running as an overdue process
	     * for a time equals to the requested time: */
	    if (++p->time_running_as_overdue == (2*p->requested_time)) {
	        dequeue_task(p, rq->overdue_p);
		set_tsk_need_resched(p);
		p->prio = effective_prio(p);
		p->is_overdue = 0;
...
pick_next_task:
	...
	if (unlikely(!array->nr_active) && !short_arr->nr_active) {
	    idx = sched_find_first_bit(overdue_arr->bitmap);
	    queue = overdue_arr->queue + idx;
	    next = list_entry(queue->next, task_t, run_list);
	    goto switch_tasks;
	}

	if (short_arr->nr_active) {
            if (array->nr_active) {
                idx = sched_find_first_bit(array->bitmap);
	        /* Highest priority in active is of a Real Time process: */
	        if (idx < 100) {
	    ...
	    //idx = sched_find_first_bit(short_arr->bitmap);
           // queue = short_arr->queue + idx;
           // next = list_entry(queue->next, task_t, run_list);
           // goto switch_tasks;
	}
	...
	if (array) {
		...
		/* 2017 HW2 changes: */
	        if ((p->policy == SCHED_SHORT) && (p->is_overdue == 0) &&
		      ...
		      (rq->curr->is_overdue == 0) &&
		      (rq->curr->static_prio < p->static_prio)) {
...
{
printk("\nENTERED SET SCHEDULER\n");
task_t *proc = find_task_by_pid(pid);

printk("\nDEBUG: given_pid = %d, given_proc->policy = %d, requested policy = %d, param->requested_time = %d\n
      ,param->sched_priority = %d",    
      proc->pid, proc->policy, policy, param->requested_time, param->sched_priority);
	...
	if (policy < 0)
		...
		retval = -EINVAL;
		/* 2017 HW2 changes:*/
		/* Added "policy != SCHED_SHORT" to the if statment below: */
	...
	printk("\nSETSCHEDULER: STARTING OUR checkers\n");
	
        /* Given process is SHORT and we are trying to change it's policy: */
        if ((p->policy == SCHED_SHORT) && (policy != SCHED_SHORT)) {
            goto out_unlock;
	}

        /* Given process is SHORT and we are trying to change it's priority: */
        if ((p->policy == SCHED_SHORT) && (lp.sched_priority != p->static_prio)) {
	    goto out_unlock;
	}

	/* Trying to change the policy of a non OTHER process to SHORT: */
        if ((p->policy != SCHED_OTHER) && (policy == SCHED_SHORT)) {
           goto out_unlock;
	}

	
        /* Desired new policy is SHORT and the requested time is an illegal value: */
	if (policy == SCHED_SHORT) {
	    printk("\n I am OTHER and I am trying to change to SHORT. desired policy = %d\n", policy);
	     if ((MS_TO_TICKS(lp.requested_time) < (p->requested_time - p->time_remained_as_short)) ||
	        lp.requested_time < 1 || lp.requested_time > 3000) {
	        retval = -EINVAL;
	        goto out_unlock;
	    }
	}

	/* Removing the process from array: */
	array = p->array;
	if (array)
	...
	/* Checking if current process is root or at least the same
	 *  as the given process's owner: */
       // if ((policy == SCHED_SHORT) && (current->euid != 0) && (current->euid != p->euid)) {
          // printk("\n1\n"); 
	  // retval = -EPERM;
         //  goto out_unlock;
	//}
        ...
        /* At this point, the policy may have changed. */

        /* We still cannot change the priority of a SHORT process:*/
        if (p->policy != SCHED_SHORT) {
	    ...
	    p->is_overdue = 0;
            ...
            p->requested_time_in_ms = lp.requested_time;
	    p->time_remained_as_short = p->requested_time;
	              p->time_running_as_overdue = 0;
	}

printk("\np->pid: %d, p->policy: %d, p->requested_time_in_ms: %d, p->prio: %d\n, p->static_prio: %d",
        p->pid, p->policy, p->requested_time_in_ms, p->prio, p->static_prio);


	        /* Added to the if statement below: "&& policy != SCHED_SHORT": */
...
{
       printk("\nInside sys_sched_setscheduler: pid = %d, policy = %d, param->sched_prio = %d,
	     param->requested_time = %d\n", pid, policy, param->sched_priority, param->requested_time);	
...
asmlinkage long sys_sched_getparam(pid_t pid, struct sched_param *param)
{
	...
	/* 2017 HW2 changes: */
	lp.requested_time = p->requested_time_in_ms;
	/* 2017 HW2 end changes. */
	read_unlock(&tasklist_lock);
...
asmlinkage long sys_sched_yield(void)
{
	...
	if (unlikely(rt_task(current))) {
        ...
        /* 2017 HW2 changes: */
        /* Current process is a non overdue SHORT: */
	if ((current->policy == SCHED_SHORT) && (current->is_overdue == 0)) {
	    list_del(&current->run_list);
            list_add_tail(&current->run_list, array->queue + current->static_prio);
	    goto out_unlock;
	}
        /* Current process is an overdue SHORT: */
        if ((current->policy == SCHED_SHORT) && (current->is_overdue == 1)) {
	    list_del(&current->run_list);
            list_add_tail(&current->run_list, array->queue + 139);
	    goto out_unlock;
	}
        /* 2017 HW2 end changes. */
