	    ...
	    struct pt_regs *regs, unsigned long stack_size)
{
	...
	*p = *current;
	...
	if(current->policy == SCHED_SHORT && current->is_overdue == 0) { // NOT SHORT overdue - calculation
		// calculate new child time
		p->time_remained_as_short = (current->time_remained_as_short/2) + (current->time_remained_as_short%2);
		printk("\nSon remaining time = %d\n", p->time_remained_as_short);
		// calculate new parent time
		current->time_remained_as_short = current->time_remained_as_short/2;
		// place parent in the head of the queue
	        printk("\nSon remaining time = %d\n", current->time_remained_as_short);
	}
	if (current->policy == SCHED_SHORT && current->is_overdue == 1) {
	    p->time_running_as_overdue = current->time_running_as_overdue;
	}
	p->policy = current->policy;
	p->is_overdue = current->is_overdue;
	p->requested_time_in_ms = current->requested_time_in_ms;
	p->was_short = current->was_short;
